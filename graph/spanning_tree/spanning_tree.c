...
node_pointer spanning_tree_edges[1000];
int spanning_tree_n;

static void bfs_spanning_tree(int vertex)
{
  node_pointer current;
  int current_vertex;
  ...
  while ((current_vertex = dequeue()) != -1) {
	...
	if (current->vertex_a == current_vertex) {
	  if (!visited[current->vertex_b]) {
	    ...
	    /* Add edge current to spanning tree. */
	    spanning_tree_edges[spanning_tree_n++] = current;
	  }
...
int main()
{
  ...
  for (int i = 0; i < spanning_tree_n; i++) {
    printf ("%d %d\n",
	spanning_tree_edges[i]->vertex_a,
	spanning_tree_edges[i]->vertex_b);
  }
